...
def test_sap_enrichment_node_prefers_contact_scorer_odata() -> None:
    ...
    assert update["extracted_data"] is not None
    assert (
        audit_text(update["extracted_data"].economic_operator.electronic_contact_details)
        == "anna.schmidt@supplier.example"
    )
    assert update["enrichment_result"] is not None
    assert "economic_operator.electronic_contact_details" in update["enrichment_result"].filled_field_paths
